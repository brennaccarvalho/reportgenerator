...
def load_reports() -> pd.DataFrame:
    """Carrega todos os relatórios do banco de dados."""
    try:
        conn = get_connection()
        rows = conn.execute(
...
def delete_report(report_id: str, path: str) -> None:
    """Remove relatório do banco e do sistema de arquivos."""
    try:
        conn = get_connection()
    ...
    for _, row in filtered.iterrows():
        ...
        with st.container():
            ...
            with c1:
                st.markdown(f"**{name}**")
                st.markdown(
                    f"Framework: `{framework}` | Criado em: {created} | "
                    f"Fonte: {source} | {n_rows} linhas × {n_cols} colunas"
                )
            ...
            with c3:
                if st.button("Deletar", key=f"del_{report_id}", type="secondary"):
